...
function setup() {
    ...
    i=1
    until [ "$(ldapsearch -x -h ${CONTAINER_IP} -b ou=snapshots,${BASE_DN} -D cn=admin,${BASE_DN} -w admin | grep 'result:')" = "result: 0 Success" ]
    do
        sleep 1
        i=$(( i+1 ))
        if [ $i -gt 100 ]
...
@test "check admin" {
    run ldapsearch -h ${CONTAINER_IP} -D cn=admin,${BASE_DN} -w admin \
        -b cn=admin,${BASE_DN}
    assert_success
}
...
@test "check readonly user" {
    ...
    run ldapsearch -h ${CONTAINER_IP} -D cn=readonly,${BASE_DN} -w readonly \
        -b cn=readonly,${BASE_DN}
    assert_success
}
...
@test "check acl roles" {
    ...
    run ldapsearch -LLL -h ${CONTAINER_IP} -D cn=admin,${BASE_DN} -w admin \
